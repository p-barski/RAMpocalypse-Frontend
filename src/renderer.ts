...
export class Renderer implements RenderingService {
  ...
  private drawEntities(): void {
    ...
    for (const entity of entities) {
      this.drawEntity(entity, scale);
    }
  }

  private drawEntity(entity: Entity, scale: number, offsetX = 0, offsetY = 0, offsetAngle = 0): void {
    ...
    this.drawEntityHealthBar(entity, canvasX, canvasY, scale);
  ...
  private drawEntityHealthBar(entity: Entity, canvasX: number, canvasY: number, scaleX: number): void {
    ...
    this.drawHealthBar(canvasX, canvasY - this.HEALTH_BAR_OFFSET_Y * scaleX, entity.width * scaleX, health, maxHealth);
  }
